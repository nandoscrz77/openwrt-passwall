...
local function fetch_rule(rule_name,rule_type,url,exclude_domain)
	...
	for k,v in ipairs(url) do
		...
		if sret_tmp == 200 and non_file_check(download_file_tmp..k, vali_file..k) then
			log(rule_name.. " No." ..k.. "rules:" ..v.. "An error occurred while downloading the file. Please try downloading again.。")
			...
			if sret_tmp == 200 and non_file_check(download_file_tmp..k, vali_file..k) then
				sret = 0
				sret_tmp = 0
				log(rule_name.. " No." ..k.. "rules:" ..v.. "An error occurred while downloading the file. Please check the network or download link and try again.！")
			end
		end
		...
		if sret_tmp == 200 then
			...
			sret = 0
			log(rule_name.. " No." ..k.. "rules:" ..v.. "Download failed, please check the network or download link and try again！")
		end
		os.remove(download_file_tmp..k)
		os.remove(vali_file..k)
	end
	...
	if sret == 200 then
		...
		if old_md5 ~= new_md5 then
			...
			reboot = 1
			log(rule_name.. " Update successful, total number of rules " ..count.. " strip。")
		else
